...
def generate_random_matrix_and_inner_product(S, dim_S):
    """
    0 ~ q 까지 범위의 dim_S x dim_S matrix A 생성 및 A 내적 S 실행.
    """
    A = np.random.randint(0, 8190, size=(dim_S, dim_S))             # (dim_S) x (dim_S) 인 0 ~ q 범위의 A matrix 생성
    S_array = np.array(S)
...
A, A_S = generate_random_matrix_and_inner_product(S, dim_S)         # A 생성 및 A 내적 S 생성, dim_S 계산
q = 8192                                                            # q 생성
M = 165                                                             # message 생성
